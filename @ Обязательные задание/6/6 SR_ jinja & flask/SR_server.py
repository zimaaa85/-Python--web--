...
app.config['DATABASE'] = 'static/db/books.db'  
...
@app.route('/main/')
def main():
    baseObject = SR_Base.BookDB(get_connect())
    books = baseObject.getALLBooks()
    return render_template('main.html', menu=navMenu, books=books)

@app.route('/info/')
def info():
    return render_template('info.html', menu=navMenu)

# обработчик адреса продукта url
@app.route('/book/<int:id>')
def showBook(id: int):    
    base = SR_Base.BookDB(get_connect())
    res = base.getBookFromId(id)
    if res:
        return render_template('concretBook.html', book=res, menu=navMenu)
    else:
        return f'товар потерян, всё сломалось'
